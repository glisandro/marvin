...
class Receiving extends CI_Model
{
	...
	*/ 
	...
	function get_suspended_receivings_for_room($room_id)
	{
		$this->db->from('receivings');
		$this->db->join('receivings_bedrooms', 'receivings.receiving_id = receivings_bedrooms.receiving_id');
		$this->db->where('receivings.suspended', '1');
		$this->db->where('receivings_bedrooms.room_id', $room_id);
