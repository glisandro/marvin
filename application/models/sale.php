		...
		foreach($payments_by_sale as $sale_id => $payment_rows)
		{
			if (isset($sales_totals[$sale_id]))
			{
				...
				foreach($payment_rows as $row)
				{
					...
					if (!isset($payment_data[$sale_id][$row['payment_type']]))
					{
						$payment_data[$sale_id][$row['payment_type']] = array('sale_id' => $sale_id,'payment_type' => $row['payment_type'], 'payment_amount' => 0,'payment_date' => $row['payment_date'], 'sale_time' => $row['sale_time'] );
					}
				
					if ($total_sale_balance != 0)
					{
						$payment_data[$sale_id][$row['payment_type']]['payment_amount'] += $payment_amount;
					}
