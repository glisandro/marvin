...
class Detailed_expenses_category extends Report
{
	...
	public function getDataColumns()
	{
		...
		$return['details'][] = array('data'=>lang('common_store'), 'align'=> 'left');
	...
	public function getData()
	{
		...
		$this->db->select($expenses.'.category_id,'.$expenses.'.expense_id,'.$category.'.name as category_name,sum('.$expenses.'.quantity) as total', false);
		...
		{
			$data['summary'][$sale_summary_row['category_id']] = $sale_summary_row; 
		}

		$categorys_ids = array();
		foreach($data['summary'] as $expense_row)
		{
			$categorys_ids[] = $expense_row['category_id'];
			
		}

		if ($this->params['category_id']=="-1" || $this->params['category_id']=="0")
		{
			$this->db->select('0 as category_id,'.$cash.'.cash_id as expense_id,"Otras" as category_name,sum('.$cash.'.quantity) as total', false);
			$this->db->from($cash);
		...
		$this->db->select($expenses.'.category_id,'.$expenses.'.expense_id,'.$locations.'.name as location_name,'.$expenses.'.expense_time,'.$expenses.'.nro_receipt,'.$expenses.'.description,'.$expenses.'.note,'.$expenses.'.quantity,'.$category.'.name as category_name,'.$people.'.first_name,'.$people.'.last_name', false);
		$this->db->from($expenses);
		$this->db->join($locations, $expenses.'.location_id = '.$locations.'.location_id');
		$this->db->join($category, $expenses.'.category_id = '.$category.'.category_id');
		$this->db->join($people, $expenses.'.approved_by_employee_id = '.$people.'.person_id');
		...
		if (!empty($categorys_ids))
		{
			$this->db->where_in($expenses.'.category_id', $categorys_ids);
		}
		else
		{
		...
		{
			$data['details'][$expense_item_row['category_id']][] = $expense_item_row;
		}	

		if ($this->params['category_id']=="-1" || $this->params['category_id']=="0")
		{

			$this->db->select('0 as category_id,'.$cash.'.cash_id as expense_id,'.$locations.'.name as location_name,'.$cash.'.cash_time as expense_time,"" as nro_receipt,'.$cash.'.description,"" as note,'.$cash.'.quantity,"Otras" as category_name,'.$people.'.first_name,'.$people.'.last_name', false);
			$this->db->from($cash);
			$this->db->join($locations, $cash.'.location_id = '.$locations.'.location_id');
			$this->db->join($people, $cash.'.approved_by_employee_id = '.$people.'.person_id');
