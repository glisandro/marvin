...
class Detailed_profit_and_loss extends Report
{
	...
	public function getData()
	{
		...
		$this->db->where('date(expense_time) BETWEEN '. $this->db->escape($this->params['start_date']). ' and '. $this->db->escape($this->params['end_date']));
		
		foreach($this->db->get()->result_array() as $sale_total_row)
		{
			$data['total_expenses'] += $sale_total_row['total'];
		}

		$this->db->select('sum(quantity) as total', false);
		$this->db->from('cash_withdrawal');
		$this->db->where('deleted',0);
		$this->db->where('date(cash_time) BETWEEN '. $this->db->escape($this->params['start_date']). ' and '. $this->db->escape($this->params['end_date']));
