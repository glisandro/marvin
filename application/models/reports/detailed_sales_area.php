...
class Detailed_sales_area extends Report
{
	...
	public function getData()
	{
		...
		if ($this->params['area']!="-1")
			$this->db->where('customer.city LIKE "%'.rawurldecode($this->params['area']).'%"');
		if ($this->params['area2']!="-1")
			$this->db->where('customer.address_2 LIKE "%'.rawurldecode($this->params['area2']).'%"');
		...
		$this->db->order_by('customer.city,sale_date');
		...
		foreach($data['summary'] as $sale_row)
		{
			...
			if ($customer_ids!=$sale_row['customer_id'])
			{
				$customer_ids=$sale_row['customer_id'];
				$this->db->select('sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit', false);
				$this->db->from('sales_items_temp');
				$this->db->where('customer_id', $sale_row['customer_id']);
				if ($this->params['sale_type'] == 'sales')
				{
					$this->db->where('quantity_purchased > 0');
				}
				...
				$data['overall'][$sale_row['customer_id']]['subtotal'] = $return['subtotal'];
				$data['overall'][$sale_row['customer_id']]['total'] = $return['total'];
				$data['overall'][$sale_row['customer_id']]['tax'] = $return['tax'];
				$data['overall'][$sale_row['customer_id']]['profit'] = $return['profit'];
			}
		}
