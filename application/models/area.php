...
{	
	...
	*/
	...
	Get search suggestions to find customers
	*/
	function get_area_search_suggestions2($search,$limit=25)
	{
		$suggestions = array();
		...
		$this->db->where("people.address_2 LIKE '%".$this->db->escape_like_str($search)."%'  and deleted=0");			
		$this->db->group_by('people.address_2 ');
		$this->db->order_by('people.address_2 ');
		//$this->db->limit($limit);	
		$by_name = $this->db->get();
		$temp_suggestions = array();
		...
		foreach($by_name->result() as $row)
		{
			$temp_suggestions[$row->address_2] = $row->address_2;
		}
	...
	Get search suggestions to find customers
	*/
	function get_area_search_suggestions($search,$limit=25)
	{
		$suggestions = array();
		...
		$this->db->where("people.city LIKE '%".$this->db->escape_like_str($search)."%'  and deleted=0");			
		$this->db->group_by('people.city ');
		$this->db->order_by('people.city ');
		//$this->db->limit($limit);	
		$by_name = $this->db->get();
		$temp_suggestions = array();
		...
		foreach($by_name->result() as $row)
		{
			$temp_suggestions[$row->city] = $row->city;
		}
