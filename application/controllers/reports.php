...
{	
	...
	function sales_customer_data_input()
	{
		$data = $this->_get_common_report_data(TRUE);
		...
		$this->load->view("reports/sales_customer_data_input",$data);	
	}

	function expenses_location_data_input()
	{
		$data = $this->_get_common_report_data(TRUE);
		$data['specific_input_name'] = lang('reports_location');
		$suggestions = $this->Location->get_location_search_suggestions($this->input->get('term'),100);
		$location=array();
		$location['']='Todos';
		foreach($suggestions as $option)
		{
			$location[$option['value']]=$option['label'];
		}
		$data['specific_input_data'] = $location;
		

		$this->load->view("reports/expenses_location_data_input",$data);		
	}

	function location_data_input()
	{
		$data = $this->_get_common_report_data(TRUE);
		$data['specific_input_name'] = lang('reports_location');
		$suggestions = $this->Location->get_location_search_suggestions($this->input->get('term'),100);
		...
		$this->load->view("reports/location_date_input",$data);		
	}

	function expenses_category_data_input()
	{
		$data = $this->_get_common_report_data(TRUE);
		$data['specific_input_name'] = lang('reports_category');
		$suggestions = $this->Category->get_category_search_suggestions($this->input->get('term'),100);
		$category=array();
		$category['']='Todos';
		foreach($suggestions as $option)
		{
			$category[$option['value']]=$option['label'];
		}
		$category['0']='Otras';
		$data['specific_input_data'] = $category;
		

		$this->load->view("reports/expenses_category_data_input",$data);		
	}

	function category_data_input()
	{
		$data = $this->_get_common_report_data(TRUE);
		$data['specific_input_name'] = lang('reports_category');
		$suggestions = $this->Category->get_category_search_suggestions($this->input->get('term'),100);
		...
		$this->load->view("reports/category_date_input",$data);		
	}




	function sales_area_data_input()
	{
		$data = $this->_get_common_report_data(TRUE);
		$data['specific_input_name'] = lang('reports_area');
		$suggestions = $this->Area->get_area_search_suggestions($this->input->get('term'),100);
		$area=array();
		$area['']='Todos';
		foreach($suggestions as $option)
		{
			$area[$option['value']]=$option['value'];
		}
		$data['specific_input_data'] = $area;
		

		$data['specific_input_name2'] = lang('reports_area_2');
		$suggestions2 = $this->Area->get_area_search_suggestions2($this->input->get('term'),100);
		$area2=array();
		$area2['']='Todos';
		foreach($suggestions2 as $option)
		{
			$area2[$option['value']]=$option['value'];
		}
		
		$data['specific_input_data2'] = $area2;
		
		
		$this->load->view("reports/sales_area_data_input",$data);	
	}

	function items_less_sales_data_input()
	{
		$data = $this->_get_common_report_data(TRUE);
		$data['specific_input_name'] = lang('reports_items');
		$data['search_suggestion_url'] = site_url('reports/items_search');
		
		
		$this->load->view("reports/items_less_sales_data_input",$data);	
	}

	function count_items_data_input()
	{
		$data = $this->_get_common_report_data(TRUE);
		$data['specific_input_name'] = lang('reports_items');
		$data['search_suggestion_url'] = site_url('reports/items_search');
		...
		$this->load->view("reports/count_items_data_input",$data);	
	}
	...
	function graphical_summary_expenses_category($start_date, $end_date, $category_id)
	{
		$this->check_action_permission('view_expenses');
		$start_date=rawurldecode($start_date);
		$end_date=date('Y-m-d 23:59:59', strtotime(rawurldecode($end_date)));
		
		$this->load->model('reports/Summary_expenses_category');
		$model = $this->Summary_expenses_category;
		$model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'category_id' => $category_id));
		
		$data = array(
			"title" => lang('reports_expenses_category_summary_reports'),
			"graph_file" => site_url("reports/graphical_summary_expenses_category_graph/$start_date/$end_date/$category_id"),
			"subtitle" => date(get_date_format(), strtotime($start_date)) .'-'.date(get_date_format(), strtotime($end_date)),
			"summary_data" => $model->getSummaryData()
		);
	...
	function graphical_summary_expenses_category_graph($start_date, $end_date, $category_id)
	{
		$start_date=rawurldecode($start_date);
		$end_date=date('Y-m-d 23:59:59', strtotime(rawurldecode($end_date)));
		...
		$report_data = $model->getData();
		$report_data_cash = $model->getDataCash();
//var_dump($report_data);die();
		$graph_data = array();
		foreach($report_data as $row)
		{
			$graph_data[$row['category_name']]= $row['total'];
		}
		foreach($report_data_cash as $row)
		{
			$graph_data[$row['category_name']]= $row['total'];
		}
//var_dump($graph_data);die();
		$data = array(
			"title" => lang('reports_expenses_category_summary_reports'),
			"data" => $graph_data
		);
	...
	function graphical_summary_expenses_location($start_date, $end_date, $location_id)
	{
		$this->check_action_permission('view_expenses');
		$start_date=rawurldecode($start_date);
		$end_date=date('Y-m-d 23:59:59', strtotime(rawurldecode($end_date)));
		
		$this->load->model('reports/Summary_expenses_location');
		$model = $this->Summary_expenses_location;
		$model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'location_id' => $location_id));
		
		$data = array(
			"title" => lang('reports_expenses_location_summary_reports'),
			"graph_file" => site_url("reports/graphical_summary_expenses_location_graph/$start_date/$end_date/$location_id"),
			"subtitle" => date(get_date_format(), strtotime($start_date)) .'-'.date(get_date_format(), strtotime($end_date)),
			"summary_data" => $model->getSummaryData()
		);
	...
	function graphical_summary_expenses_location_graph($start_date, $end_date, $location_id)
	{
		$start_date=rawurldecode($start_date);
		$end_date=date('Y-m-d 23:59:59', strtotime(rawurldecode($end_date)));
...
//var_dump($report_data);die();
		...
		foreach($report_data as $row)
		{
			$graph_data[$row['location_name']]= $row['total'];
		}

		foreach($report_data_cash as $row)
		{
			if (isset($graph_data[$row['location_name']]))
			{
				$row['total']+=$graph_data[$row['location_name']];	
			}
			$graph_data[$row['location_name']]= $row['total'];
		}
//var_dump($graph_data);die();
	...
	function detailed_sales_customer($start_date, $end_date, $customer_id, $sale_type, $export_excel=0, $offset = 0)
	{				
		$this->check_action_permission('view_sales');
		$start_date=rawurldecode($start_date);
		$end_date=rawurldecode($end_date);

		$this->load->model('reports/Detailed_sales_customer');
		$model = $this->Detailed_sales_customer;
		$model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'customer_id' =>$customer_id, 'sale_type' => $sale_type, 'offset' => $offset, 'export_excel' => $export_excel));
		
		$this->Sale->create_sales_items_temp_table(array('start_date'=>$start_date, 'end_date'=>$end_date, 'customer_id' =>$customer_id, 'sale_type' => $sale_type));
		$config = array();
		$config['base_url'] = site_url("reports/detailed_sales_customer/".rawurlencode($start_date).'/'.rawurlencode($end_date)."/$customer_id/$sale_type/$export_excel");
		$config['total_rows'] = $model->getTotalRows();
		$config['per_page'] = $this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20; 
		$config['uri_segment'] = 8;
		$this->pagination->initialize($config);
		...
		foreach($report_data['summary'] as $key=>$row)
		{
			...
			if ($IdCustomer!=$row['customer_id'])
			{
				$summary_data_row[] = array('data'=>'<a href="'.$link.'" target="_blank">'.$row['customer_name'].'</a>', 'align'=>'left');
				$IdCustomer=$row['customer_id'];
				$summary_data['head'.$IdCustomer] = $summary_data_row;
			...
			// else
			// {
				$summary_data_row[] = array('data'=>'', 'align'=>'left');
				$summary_data_row[] = array('data'=>anchor('sales/receipt/'.$row['sale_id'], '<i class="fa fa-print fa fa-2x"></i>', array('target' => '_blank', 'class'=>'hidden-print')).'<span class="visible-print">'.$row['sale_id'].'</span>'.anchor('sales/edit/'.$row['sale_id'], '<i class="fa fa-file-alt fa fa-2x"></i>', array('target' => '_blank')).' '.anchor('sales/edit/'.$row['sale_id'], lang('common_edit').' '.$row['sale_id'], array('target' => '_blank','class'=>'hidden-print')), 'align'=>'left');
				$summary_data_row[] = array('data'=>date(get_date_format().'-'.get_time_format(), strtotime($row['sale_time'])), 'align'=>'left');
				$summary_data_row[] = array('data'=>$row['register_name'], 'align'=>'left');
				$summary_data_row[] = array('data'=>to_quantity($row['items_purchased']), 'align'=>'left');
		...
		$data = array(
			"title" =>lang('reports_detailed_sales_customer_report'),
			"subtitle" => date(get_date_format(), strtotime($start_date)) .'-'.date(get_date_format(), strtotime($end_date)),
			"headers" => $model->getDataColumns(),
			"summary_data" => $summary_data,
			"details_data" => $details_data,
			"details_overall_data" => $report_data['overall'],
			"overall_summary_data" => $model->getSummaryData(),
			"export_excel" => $export_excel,
			"pagination" => $this->pagination->create_links(),
		);

		$this->load->view("reports/tabular_details_sales_customer",$data);
	}

	function detailed_sales_area($start_date, $end_date, $area,$area2, $sale_type, $export_excel=0, $offset = 0)
	{				
		$this->check_action_permission('view_sales');
		$start_date=rawurldecode($start_date);
		$end_date=rawurldecode($end_date);

		$this->load->model('reports/Detailed_sales_area');
		$model = $this->Detailed_sales_area;
		$model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'area' =>$area, 'area2' =>$area2, 'sale_type' => $sale_type, 'offset' => $offset, 'export_excel' => $export_excel));
		
		$this->Sale->create_sales_items_temp_table(array('start_date'=>$start_date, 'end_date'=>$end_date, 'area' =>$area, 'area2' =>$area2, 'sale_type' => $sale_type));
		$config = array();
		$config['base_url'] = site_url("reports/detailed_sales_area/".rawurlencode($start_date).'/'.rawurlencode($end_date)."/$area/$area2/$sale_type/$export_excel");
		$config['total_rows'] = $model->getTotalRows();
		$config['per_page'] = $this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20; 
		$config['uri_segment'] = 8;
		$this->pagination->initialize($config);
		...
		foreach($report_data['summary'] as $key=>$row)
		{
			...
			if (strtoupper($AreaStr)!=strtoupper(trim($row['city'])).strtoupper(trim($row['address_2'])))
			{
				$summary_data_row[] = array('data'=>'<b>Departamento</b>: '.strtoupper($row['city']).' <b>Municipio</b>:'.strtoupper($row['address_2']), 'align'=>'left');
				$AreaStr=trim($row['city']).trim($row['address_2']);
				$summary_data['headCity'.$AreaStr] = $summary_data_row;
			}
			$summary_data_row = array();
		...
		$data = array(
			"title" =>lang('reports_detailed_sales_area_report'),
			"subtitle" => date(get_date_format(), strtotime($start_date)) .'-'.date(get_date_format(), strtotime($end_date)),
			"headers" => $model->getDataColumns(),
			"summary_data" => $summary_data,
			"details_data" => $details_data,
		...
		$this->load->view("reports/tabular_details_sales_area",$data);
	}

	function detailed_items_less_sales($start_date, $end_date, $item, $sale_type, $export_excel=0, $offset = 0)
	{				
			$this->check_action_permission('view_items');
		$start_date=rawurldecode($start_date);
		$end_date=rawurldecode($end_date);

		$this->load->model('reports/Detailed_items_less_sales');
		$model = $this->Detailed_items_less_sales;
		$model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'item' =>$item, 'sale_type' => $sale_type, 'offset' => $offset, 'export_excel' => $export_excel));

		$this->Sale->create_sales_items_temp_table(array('start_date'=>$start_date, 'end_date'=>$end_date, 'item' =>$item, 'sale_type' => $sale_type));

		$config = array();
		$config['base_url'] = site_url("reports/detailed_items_less_sales/".rawurlencode($start_date).'/'.rawurlencode($end_date)."/$item/$sale_type/$export_excel");
		$config['total_rows'] = $model->getTotalRows();
		$config['per_page'] = $this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20; 
		$config['uri_segment'] = 8;
		...
		foreach($report_data as $row)
		{
			...
			// $data_row[] = array('data'=>$row['product_id'], 'align' => 'left');
			// $data_row[] = array('data'=>$row['category'], 'align' => 'left');
			$data_row[] = array('data'=>to_quantity($row['quantity_purchased']), 'align' => 'center');
			// $data_row[] = array('data'=>to_currency($row['subtotal']), 'align' => 'right');
			// $data_row[] = array('data'=>to_currency($row['total']), 'align' => 'right');
			// $data_row[] = array('data'=>to_currency($row['tax']), 'align' => 'right');
			if($this->has_profit_permission)
		...
		$data = array(
			"title" => lang('reports_items_less_sales_report'),
			"subtitle" => date(get_date_format(), strtotime($start_date)) .'-'.date(get_date_format(), strtotime($end_date)),
			"headers" => $model->getDataColumns(),
			"data" => $tabular_data,
			"summary_data" => $model->getSummaryData(),
		...
		$this->load->view("reports/tabular_details_items_less_sales",$data);
	}

	function detailed_expenses_location($start_date, $end_date, $location_id, $export_excel=0, $offset = 0)
	{				
		$this->check_action_permission('view_expenses');
		$start_date=rawurldecode($start_date);
		$end_date=rawurldecode($end_date);

		$this->load->model('reports/Detailed_expenses_location');
		$model = $this->Detailed_expenses_location;
		$model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'location_id' =>$location_id, 'offset' => $offset, 'export_excel' => $export_excel));
		
		$config = array();
		$config['base_url'] = site_url("reports/detailed_expenses_location/".rawurlencode($start_date).'/'.rawurlencode($end_date)."/$location_id/$export_excel");
		$config['total_rows'] = $model->getTotalRows();
		$config['per_page'] = $this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20; 
		$config['uri_segment'] = 7;
		$this->pagination->initialize($config);
		...
		foreach($report_data['summary'] as $key=>$row)
		{
			...
			// {
				...
				foreach($report_data['details'][$key] as $drow)
				{
					...
					$details_data_row[] = array('data'=>date(get_date_format(), strtotime($drow['expense_time'])), 'align'=>'left');
					$details_data_row[] = array('data'=>$drow['nro_receipt'], 'align'=>'left');
					$details_data_row[] = array('data'=>$drow['description'], 'align'=>'left');
					$details_data_row[] = array('data'=>$drow['note'], 'align'=>'left');
					$details_data_row[] = array('data'=>to_currency($drow['quantity']), 'align'=>'right');
					$details_data_row[] = array('data'=>$drow['category_name'], 'align'=>'left');
					$details_data_row[] = array('data'=>$drow['first_name']." ".$drow['last_name'], 'align'=>'left');
		...
		$data = array(
			"title" =>lang('reports_detailed_expenses_location_reports'),
			"subtitle" => date(get_date_format(), strtotime($start_date)) .'-'.date(get_date_format(), strtotime($end_date)),
			"headers" => $model->getDataColumns(),
			"summary_data" => $summary_data,
			"details_data" => $details_data,
		...
		$this->load->view("reports/tabular_details_expenses_location",$data);
	}

	function detailed_expenses_category($start_date, $end_date, $category_id, $export_excel=0, $offset = 0)
	{				
		$this->check_action_permission('view_expenses');
		$start_date=rawurldecode($start_date);
		$end_date=rawurldecode($end_date);

		$this->load->model('reports/Detailed_expenses_category');
		$model = $this->Detailed_expenses_category;
		$model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'category_id' =>$category_id, 'offset' => $offset, 'export_excel' => $export_excel));
		
		$config = array();
		$config['base_url'] = site_url("reports/detailed_expenses_category/".rawurlencode($start_date).'/'.rawurlencode($end_date)."/$category_id/$export_excel");
		$config['total_rows'] = $model->getTotalRows();
		$config['per_page'] = $this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20; 
		$config['uri_segment'] = 7;
		$this->pagination->initialize($config);
		...
		foreach($report_data['summary'] as $key=>$row)
		{
			...
			// {
				...
				foreach($report_data['details'][$key] as $drow)
				{
					...
					$details_data_row[] = array('data'=>$drow['note'], 'align'=>'left');
					$details_data_row[] = array('data'=>to_currency($drow['quantity']), 'align'=>'right');
					$details_data_row[] = array('data'=>$drow['location_name'], 'align'=>'left');
					$details_data_row[] = array('data'=>$drow['first_name']." ".$drow['last_name'], 'align'=>'left');
		...
		$data = array(
			"title" =>lang('reports_detailed_expenses_category_reports'),
			"subtitle" => date(get_date_format(), strtotime($start_date)) .'-'.date(get_date_format(), strtotime($end_date)),
			"headers" => $model->getDataColumns(),
			"summary_data" => $summary_data,
			"details_data" => $details_data,
		...
		$this->load->view("reports/tabular_details_expenses_category",$data);
	}

	function summary_count_items( $item, $export_excel=0, $offset = 0)
	{				
			$this->check_action_permission('view_items');
		
		$this->load->model('reports/summary_count_items');
		$model = $this->summary_count_items;
		$model->setParams(array('item' =>$item, 'offset' => $offset, 'export_excel' => $export_excel));

		$this->Sale->create_sales_items_temp_table(array( 'item' =>$item));


		$config = array();
		$config['base_url'] = site_url("reports/summary_count_items/$item/$export_excel");
		$config['total_rows'] = $model->getTotalRows();
		$config['per_page'] = $this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20; 
		...
		$data = array(
			"title" => lang('reports_count_items_reports'),
			"subtitle" => '',
			"headers" => $model->getDataColumns(),
			"data" => $tabular_data,
			"summary_data" => '',
		...
		$this->load->view("reports/tabular_summary_count_items",$data);
	}
	...
	{
		$suggestions = $this->Item->get_item_search_suggestions($this->input->get('term'),100,"todos");
		echo json_encode($suggestions);
	}
	...
	function area_dos_search()
	{
		$suggestions = $this->Area->get_area_search_suggestions2($this->input->get('term'),100);
		echo json_encode($suggestions);
	}
