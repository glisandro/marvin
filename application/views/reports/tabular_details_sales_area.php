				...
				<div class="widget-content nopadding">
					...
					<table class="table table-bordered table-striped  table-condensed table-hover detailed-reports  tablesorter" id="sortable_table">
						...
						<tbody>
							...
							<?php if (substr($key,0,8)=="headCity") { ?>
							<tr>
								<td class="hidden-print"></td>
								<?php foreach ($row as $cell) { ?>
								<td colspan="<?php echo count($headers['summary']); ?>" align="<?php echo $cell['align']; ?>"><?php echo $cell['data']; ?></td>
								<?php } ?>
								
							</tr>
							<?php } else if (substr($key,0,4)=="head") { ?>
							<tr>
								<td class="hidden-print"></td>
								<?php foreach ($row as $cell) { ?>
								<td colspan="1" align="<?php echo $cell['align']; ?>"><?php echo $cell['data']; ?></td>
								<?php } ?>
								<td colspan="<?php echo count($headers['summary'])-1; ?>" align="center">
									<div class="center" style="text-align: center;">					
										<ul class="stat-boxes">
											<?php foreach($details_overall_data[substr($key,4)] as $name=>$value) { ?>
											<li class="popover-visits">
												<div class="left peity_bar_good"><h8><?php echo lang('reports_'.$name); ?></h8></div>
												<div class="right">
													<strong style="font-size:10px !important;margin-top:0px !important;margin-bottom:0px !important;"><?php echo str_replace(' ','&nbsp;', to_currency($value)); ?></strong>
												</div>
											</li>
											<?php }?>
