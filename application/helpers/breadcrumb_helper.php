...
function create_breadcrumb()
{
  	...
  	elseif ($ci->uri->segment(1) == 'expenses')
	{
		if ($ci->uri->segment(2) == false) //Main page
		{
			$customers_home_link =create_current_page_url(lang('module_expenses'));
		}
		else
		{
			$customers_home_link = '<a href="'.site_url('expenses').'">'.lang('module_expenses').'</a>';
		}
		...
		elseif($ci->uri->segment(2) == 'excel_import')
		{
			$return.=create_current_page_url(lang('expenses_import_expenses_from_excel'));
		}
		elseif($ci->uri->segment(2) == 'category')
		{
			$return.=create_current_page_url(lang('expenses_category'));
		}
		elseif($ci->uri->segment(2) == 'category_view')
		{
			if ($ci->uri->segment(3) == -1)
			{
				$customers_home_link = '<a href="'.site_url('expenses/category').'">'.lang('expenses_category').'</a>';
				$return.=$customers_home_link;
  				$return.=create_current_page_url(lang('expenses_category_new'));
			}
			else
			{
  				$customers_home_link = '<a href="'.site_url('expenses/category').'">'.lang('expenses_category').'</a>';
				$return.=$customers_home_link;
  				$return.=create_current_page_url(lang('expenses_category_update'));
			}
		}
		elseif($ci->uri->segment(2) == 'cash')
		{
			if ($ci->uri->segment(3) == -1)
			{
  				$return.=create_current_page_url(lang('expenses_cash_new'));
			}
			else
			{
  				$return.=create_current_page_url(lang('expenses_cash_update'));
			}
		}
  	}
	...
	elseif($ci->uri->segment(1) == 'bedrooms')
	{
		if ($ci->uri->segment(2) == false) //Main page
		{
			$items_home_link =create_current_page_url(lang('module_bedrooms'));
		}
		else
		{
			$items_home_link = '<a href="'.site_url('bedrooms').'">'.lang('module_bedrooms').'</a>';
		}
		...
		if($ci->uri->segment(2) == 'view')
		{
			if ($ci->uri->segment(3) == -1)
			{
  				$return.=create_current_page_url(lang('bedrooms_new'));
			}
			else
			{
  				$return.=create_current_page_url(lang('bedrooms_update'));
			}
		}
		elseif($ci->uri->segment(2) == 'excel_import')
		{
	...
	elseif($ci->uri->segment(1) == 'reports')
	{
		...
		elseif($ci->uri->segment(2) == 'detailed_sales_customer')
		{
			$return.=create_report_breadcrumb(lang('reports_detailed_sales_customer_report'));
		}	
		elseif($ci->uri->segment(2) == 'detailed_sales_area')
		{
			$return.=create_report_breadcrumb(lang('reports_detailed_sales_area_report'));
		}
		elseif($ci->uri->segment(2) == 'detailed_items_less_sales')
		{
			$return.=create_report_breadcrumb(lang('reports_items_less_sales_report'));
		}
		elseif($ci->uri->segment(2) == 'detailed_expenses_location')
		{
			$return.=create_report_breadcrumb(lang('reports_detailed_expenses_location_reports'));
		}
		elseif($ci->uri->segment(2) == 'detailed_expenses_category')
		{
			$return.=create_report_breadcrumb(lang('reports_detailed_expenses_category_reports'));
		}	
		elseif($ci->uri->segment(2) == 'graphical_summary_expenses_category')
		{
			$return.=create_report_breadcrumb(lang('reports_expenses_category_summary_reports'));
		}
		elseif($ci->uri->segment(2) == 'graphical_summary_expenses_location')
		{
			$return.=create_report_breadcrumb(lang('reports_expenses_location_summary_reports'));
		}		
		elseif($ci->uri->segment(2) == 'summary_count_items')
		{
			$return.=create_report_breadcrumb(lang('reports_count_items_reports'));
		}		
