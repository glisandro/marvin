...
function get_items_barcode_data($item_ids)
{
	...
	foreach ($item_ids as $item_id)
	{
		...
		if($CI->config->item('barcode_price_include_tax'))
		{
			...
			{
				$result[] = array('name' => ($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;">'.to_currency($regular_item_price).'</span> ' : ' ').to_currency($item_price).'<br>'.$item_info->name, 'id'=> number_pad($item_id, 10));
			}
			else
			{				
				$result[] = array('name' => ($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;">'.to_currency(get_price_for_item_including_taxes($item_id,$regular_item_price)).'</span> ' : ' ').to_currency(get_price_for_item_including_taxes($item_id,$item_price)).'<br>'.$item_info->name, 'id'=> number_pad($item_id, 10));
