...
function enable_delete(confirm_message,none_selected_message)
{
	...
	$('#delete_expenses').click(function(event)
	{
		event.preventDefault();
		if($("#sortable_table tbody :checkbox:checked").length >0)
		{
			if(confirm(confirm_message))
			{
				do_delete_expenses($("#delete_expenses").attr('href'));
			}
		}
	});
}
...
function do_delete_expenses(url)
{
	...
	var row_types = get_selected_types();
	var selected_rows = get_selected_rows();
	$.post(url, { 'ids[]': row_ids,'types[]': row_types },function(response)
	{
		//delete was successful, remove checkbox rows
		if(response.success)
		{
			if (response.redirect){location.reload();}
...
function get_selected_types()
{
	...
	$("#sortable_table tbody :checkbox:checked").each(function()
	{
		selected_values.push($(this).attr('type_ctrol'));
	});
	return selected_values;
}
...
function determine_checkbox_status()
{
	if ($("#sortable_table tbody :checkbox:checked").length > 0)
	{
		...
		if ($("#delete_expenses").length>0)
			$("#delete_expenses").removeClass("disabled");
	...
	else
	{
		...
		if ($("#delete_expenses").length>0)
			$("#delete_expenses").addClass("disabled");
